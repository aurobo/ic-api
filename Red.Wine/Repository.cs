...
namespace Red.Wine
{
    public class Repository<TEntity> where TEntity : BaseModel
    {
        ...
        public Repository(DbContext context, string userId)
        {
            _context = context;
            ...
            foreach (var propertyInfo in propertyInfoArray)
            {
                try
                {
                    ...
                    if (propertyInfo.IsCollectionProperty())
                    {
                        ...
                        var toRepositoryType = typeof(Repository<>).MakeGenericType(toCollectionItemType);
                        ...
                        if (copyToAttribute.Relationship == Relationship.Dependency)
                        {
                            ...
                            else if (fromCollectionItemType == typeof(BaseModel))
                            {
                                foreach (BaseModel item in fromCollection)
                                {
                        ...
                        else if (copyToAttribute.Relationship == Relationship.Seed)
                        {
                            ...
                            var fromRepositoryType = typeof(Repository<>).MakeGenericType(copyToAttribute.From);
                    ...
                    else if (propertyInfo.IsNormalProperty())
                    {
                        if (copyToAttribute != null)
                        {
                            if (copyToAttribute.IsForeignKey)
                            {
                                ...
                                {
                                    var repositoryType = typeof(Repository<>).MakeGenericType(copyToAttribute.To);
                        ...
                        if (to.GetType().GetProperty(propertyInfo.Name).PropertyType.IsEnum)
                        {
                            ...
                            MethodInfo methodInfo = typeof(Extensions).GetMethod("ToEnum");
        ...
        {
            var entries = _context.ChangeTracker.Entries<BaseModel>().Where(x => x.State == EntityState.Modified || x.State == EntityState.Added);
